...
HRESULT Atmosphere::OnD3D11CreateDevice()
{
	...
	for (int i = 0; i < MatrixVarStr.size(); i++)
	{
		auto pVectorVar = pAtmosphereEffect->GetVariableByName(MatrixVarStr[i].c_str())->AsVector();
	...
	GeometryGenerator::MeshData groundMesh = geometryGenerator.CreateSphere(50.f, 100, 100);
	groundIndexNum = groundMesh.Indices32.size();
	V_RETURN(D3dBufferDesc::BufferDescVertex(pd3dDevice, &pGroundVB, groundMesh.Vertices.data(), groundMesh.Vertices.size() * sizeof(GeometryGenerator::Vertex)));
...
void Atmosphere::Render(D3DXMATRIX& viewProj,D3DXVECTOR3& v3CameraPos)
{
	fCameraHeight = v3CameraPos.z;
	fCameraHeight2 = fCameraHeight*fCameraHeight;
	...
	//activeTech = AtmosphereTechMap["SkyFromAtmosphere"];
	activeTech = AtmosphereTechMap["Test"];
	...
	for(UINT p = 0;p < techDesc.Passes; ++p)
	{
		...
		VectorVarMap["v3CameraPos"]->SetFloatVector(v3CameraPos);
		VectorVarMap["v3LightPos"]->SetFloatVector(SunPos);
		VectorVarMap["v3InvWavelength"]->SetFloatVector(fInvWavelength4);
