...
Vertex GeometryGenerator::MidPoint(const Vertex& v0, const Vertex& v1)
{
    ...
    D3DXVECTOR3 n0 = v0.Normal;
    D3DXVECTOR3 n1 = v1.Normal;

    D3DXVECTOR3 tan0 = v0.TangentU;
    D3DXVECTOR3 tan1 = v1.TangentU;

    D3DXVECTOR2 tex0 = v0.TexC;
    D3DXVECTOR2 tex1 = v1.TexC;

    // Compute the midpoints of all the attributes.  Vectors need to be normalized
    // since linear interpolating can make them not unit length.  
    D3DXVECTOR3 pos = 0.5f*(p0 + p1);
    D3DXVECTOR3 normal = 0.5f*(n0 + n1);
    D3DXVECTOR3 tangent = 0.5f*(tan0+tan1);
    D3DXVECTOR2 tex = 0.5f*(tex0 + tex1);

    Vertex v;
    v.Position = pos;
	D3DXVec3Normalize(&v.Normal,&normal);
    D3DXVec3Normalize(&v.TangentU,&tangent);
    v.TexC = tex;
...
GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions)
{
	...
	for(uint32 i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		D3DXVECTOR3 n;
		D3DXVec3Normalize(&n,&meshData.Vertices[i].Position);
		...
		meshData.Vertices[i].TexC.x = theta/(2 * D3DX_PI);
		meshData.Vertices[i].TexC.y = phi/(2 * D3DX_PI);
		...
		D3DXVec3Normalize(&meshData.Vertices[i].TangentU,&meshData.Vertices[i].TangentU);
	}
...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
	...
	for(uint32 i = 0; i < ringCount; ++i)
	{
		...
		// vertices of ring
		float dTheta = 2.0f*D3DX_PI/sliceCount;
		for(uint32 j = 0; j <= sliceCount; ++j)
		{
			Vertex vertex;
...
GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float depth, uint32 m, uint32 n)
{
	...
	for(uint32 i = 0; i < m; ++i)
	{
		...
		for(uint32 j = 0; j < n; ++j)
		{
			...
			meshData.Vertices[i*n+j].Position = D3DXVECTOR3(x, 0.0f, z);
			meshData.Vertices[i*n+j].Normal   = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
			meshData.Vertices[i*n+j].TangentU = D3DXVECTOR3(1.0f, 0.0f, 0.0f);
